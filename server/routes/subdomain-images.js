...
import { getSubdomain, isValidUserSubdomain, getActualUserFolder, sanitizeFilename, } from "../utils/getSubdomain";
import { getSubdomainMode } from '../index';
/**
 * Handles subdomain-based image serving
 * Route: GET /i/:filename
 *
 * Examples:
 * - https://kapoor.x02.me/i/photo.png
 * - https://kishan.x02.me/i/screenshot.jpg
 */
export const handleSubdomainImages = (req, res, next) => {
    try {
        // Extract subdomain from hostname
        const hostname = req.hostname || req.get("host") || "";
        const subdomain = getSubdomain(hostname);
        console.log(`[Subdomain Images] Request from ${hostname}, extracted subdomain: ${subdomain}`);
        ...
        if (!subdomain) {
            ...
            console.log(`[Subdomain Images] No valid subdomain found for hostname: ${hostname}`);
        ...
        if (!isValidUserSubdomain(subdomain)) {
            console.log(`[Subdomain Images] User folder not found for subdomain: ${subdomain}`);
        ...
        if (!actualFolder) {
            console.log(`[Subdomain Images] Could not determine actual folder for subdomain: ${subdomain}`);
        ...
        if (!absoluteFilePath.startsWith(expectedBasePath)) {
            console.log(`[Subdomain Images] Path traversal attempt detected: ${absoluteFilePath}`);
            return res.status(400).json({
                success: false,
        ...
        console.log(`[Subdomain Images] Serving file: ${absoluteFilePath} (${stats.size} bytes)`);
        // Serve the file
        res.sendFile(absoluteFilePath, (err) => {
            if (err) {
                ...
                console.log(`[Subdomain Images] Successfully served: ${filename} to ${subdomain}.x02.me`);
            }
        });
    }
    catch (error) {
 ...
 * Route: GET /i (when accessed via subdomain)
 */
export const listSubdomainImages = (req, res) => {
    try {
        const hostname = req.hostname || req.get("host") || "";
        const subdomain = getSubdomain(hostname);
        ...
        const images = files
            ...
            return {
                filename: file,
                url: `https://${subdomain}.x02.me/i/${file}`,
                size: stats.size,
