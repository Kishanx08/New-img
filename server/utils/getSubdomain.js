 ...
 */
export function isValidUserSubdomain(subdomain) {
    if (!subdomain)
        return false;
    ...
    return userDirs.some(dir => dir.toLowerCase() === subdomain.toLowerCase() &&
        fs.statSync(path.join(usersDir, dir)).isDirectory());
}
/**
 * Gets the actual case-sensitive folder name for a subdomain
 */
export function getActualUserFolder(subdomain) {
    if (!subdomain)
        return null;
    const usersDir = path.join("uploads", "users");
    if (!fs.existsSync(usersDir))
        return null;
    const userDirs = fs.readdirSync(usersDir);
    const actualFolder = userDirs.find(dir => dir.toLowerCase() === subdomain.toLowerCase() &&
        fs.statSync(path.join(usersDir, dir)).isDirectory());
